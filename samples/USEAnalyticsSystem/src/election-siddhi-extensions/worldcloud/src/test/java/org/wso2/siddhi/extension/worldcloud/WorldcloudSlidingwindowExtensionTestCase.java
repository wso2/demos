...
import org.wso2.siddhi.extension.worldcloud.test.util.SiddhiTestHelper;
import org.wso2.siddhi.core.util.EventPrinter;

public class WorldcloudSlidingwindowExtensionTestCase {
    static final Logger log = Logger.getLogger(WorldcloudSlidingwindowExtensionTestCase.class);
    ...
    public void testContainsFunctionExtension() throws InterruptedException {
        log.info("WorldcloudSlidingwindowExtensionTestCase SlidingWindow TestCase ");
        SiddhiManager siddhiManager = new SiddhiManager();
        String inStreamDefinition = "@config(async = 'true')define stream inputStream (text string,Rt int ,Ft int);";
        String query = ("@info(name = 'query1') " + "from inputStream#Cloud:getCloud(1,50,text) "
        ...
        executionPlanRuntime.addCallback("query1", new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents, Event[] removeEvents) {
                ...
                for (Event inEvent : inEvents) {
                    ...
                    if (count.get() == 1) {
                        Assert.assertEquals("Value: very", inEvent.getData(0).toString().split(",")[0]);
                    }
                    if (count.get() == 2) {
